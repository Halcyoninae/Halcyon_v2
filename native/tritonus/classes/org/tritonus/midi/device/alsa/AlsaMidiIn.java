...
public class AlsaMidiIn
     ...
     */
    private final AlsaSeq m_alsaSeq;
     ...
     */
    private final int m_nSourceClient;

    /**
     * ALSA port number (belonging to m_nSourceClient) to
     * subscribe to to receive events.
     */
    private final int m_nSourcePort;

    private final AlsaMidiInListener m_listener;

    private final AlsaSeqEvent m_event = new AlsaSeqEvent();

    // used to query event for detailed information
    private final int[] m_anValues = new int[5];
    private final long[] m_alValues = new long[1];
