...
class AvExport IFrame {
 ...
 public:
  ...
  /**
   * @brief Get the size in bytes that a video/audio buffer of the given frame
   * properties would occupy if allocated.
   * @warning This methods does not guarantee that the buffer is actually
   * allocated.
   */
   ...
   * @note For audio, only linesize[0] may be set. For planar audio, each
   ...
   * This function does not allocate anything: the current frame must be already
   * initialized and allocated with the same parameters as the given frame, to
   * be ready for memcpy instructions.
   * @note It copies the frame data (i.e. the contents of the data / extended
   * data arrays), not any other properties.
   * @see copyProperties
   */
  void copyData(const IFrame& frameToRef);

  /**
   * @brief Copy all the fields that do not affect the data layout in the
   * buffers.
   * @warning The info checked when copying data of an other frame
   * (width/height, channels...) are not copied.
   */
  ...
  const unsigned char** getData() const {
    return const_cast<const unsigned char**>(_frame->data);
  }
#endif
