...
public class Encoder { /* Every Encoder has a state through completion */
    private final Gsm_State g_s = new Gsm_State();
    private final Long_term lg_term_Obj = new Long_term();
    private final Lpc lpc_Obj = new Lpc();
    private final Rpe rpe_Obj = new Rpe();
    private final Short_term sh_term_Obj = new Short_term();
    ...
    /* Reads 160 bytes */
    private final int[] input_signal = new int[160]; /* [0..159] OUT */
     ...
     *
     * @param asBuffer an 160-element (for "toast" frame format) or 320-element array
     *                 (for Microsoft frame format) with the data to encode in PCM
     *                 signed 16 bit format
     * @param abFrame  the encoded GSM frame (33 or 65 bytes, depending on the frame
